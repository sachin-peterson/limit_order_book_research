...
def get_paths(
    ...
    start_local = start.tz_convert(
        "America/New_York"
    )

    end_local = end.tz_convert(
        "America/New_York"
    )

    date_string = session.date.isoformat()

    start_string = start_local.strftime(
        "%H%M%S"
    )

    end_string = end_local.strftime(
        "%H%M%S"
    )

    filename = (
        f"{symbol}_{date_string}_"
        f"{start_string}_{end_string}_"
        f"{FILE_SCHEMA}.dbn.zst"
    )

    final_path = raw_dir / filename

    temp_path = raw_dir / filename.replace(
        ".dbn.zst",
        ".tmp.dbn.zst"
    )
...
def download_session(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            print(
                f"{date_string} downloading validation data "
            ...
            print(
                ...
                f"({final_path.stat().st_size / 1_000:.1f} KB)"
            )
...
def main() -> None:
    ...
    print()
    print(
        "Estimating validation-data cost..."
    )
    ...
    if total_cost > args.max_cost:
        raise ValueError(
            ...
            f"exceeds --max-cost ${args.max_cost:.6f}"
        )

    print()
    print(
        "Starting validation-data downloads..."
    )
    ...
    print()
    print(
        "Validation-data download range complete."
    )
